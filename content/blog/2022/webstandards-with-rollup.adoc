...
For this reason, I have now moved back to a lower-level tool which is called link:https://rollupjs.org/guide/en/[rollup.js]. With rollup.js, we can convert Node dependency into standard ESM modules. Nothing more and nothing less. You can find the full example project link:https://github.com/38leinaD/webstandards-starter[on GitHub].
