...
The short summary is that the cache-coherence feature of EclipseLink is an extension and Liberty does not ship this extension by default.
The prefered solution is replace the JPA-implementation with our own. For us, this will simply be EclipseLink but including the extension.
